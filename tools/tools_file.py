...
import tools.tools_file_motn as tools_file_motn
...
def tfs_resample_tfs(tf_list, target_fps, padding=False) -> list[dict]:
    ...
    has_repeated_t = any(
        len(item["t"]) > 1 and np.any(np.diff(np.asarray(item["t"], dtype=float)) == 0.0) for item in tf_list
    )
...
def pkl_to_traj(path) -> list[dict]:
    return pickle.load(open(path, "rb"))


def traj_to_pkl(traj_list, path) -> list[dict]:
    pkl_list = []
...
def traj_to_pkl(traj_list, path) -> None:
    with open(path, "wb") as f:
