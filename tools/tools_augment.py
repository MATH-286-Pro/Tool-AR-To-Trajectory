...
def mirror_episode(episode, plane, keep_ee_tf=False):
    ...
    axis_angle_dtype = np.asarray(episode["ee_axis_angle"]).dtype if "ee_axis_angle" in episode else mirrored_tf.dtype
...
def augment_traj(
    ...
    for plane in mirror_planes:
        base_variants.extend(mirror_episode(variant, plane) for variant in list(base_variants))
